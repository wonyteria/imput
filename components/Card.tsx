...
import { MapPin, Eye, MessageCircle, Heart, Users, Video, Clock, Crown, Lock, Unlock } from 'lucide-react';
...
const Card: React.FC<CardProps> = ({ item, onClick, isLiked, onToggleLike }) => {
  ...
  if (item.categoryType === 'minddate') {
    ...
    return (
        <div 
          onClick={onClick}
          className={`group bg-white rounded-2xl border border-slate-100 overflow-hidden hover:shadow-xl transition-all duration-300 cursor-pointer flex flex-col h-full relative ${isEnded ? '' : 'hover:-translate-y-1'}`}
        >
          ...
          <div className="p-5 flex flex-col flex-grow relative">
            ...
            <p className="text-sm text-slate-500 line-clamp-2 mb-4">{item.desc}</p>
            <div className="mt-auto flex items-center justify-between">
                <span className="text-pink-600 font-bold">{item.price}</span>
                <button className={`px-4 py-1.5 rounded-full text-xs font-bold transition-colors ${isEnded ? 'bg-slate-100 text-slate-400' : 'bg-pink-50 text-pink-600 group-hover:bg-pink-100'}`}>
                    {isEnded ? '종료됨' : '신청하기'}
                </button>
            </div>
          </div>
  ...
  if (item.categoryType === 'crew') {
    ...
    if (isReport) {
        ...
        return (
            <div onClick={onClick} className="group bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-lg cursor-pointer relative flex flex-col h-full hover:-translate-y-1 transition-transform duration-300">
                <div className="relative h-48 overflow-hidden">
                    ...
                    {/* Tags */}
                    <div className="absolute top-3 left-3 flex gap-1">
                        <span className="bg-slate-900 text-white text-[10px] font-bold px-2 py-1 rounded shadow-sm border border-slate-700">리포트</span>
                    ...
                    <div className="absolute bottom-3 left-3 right-3 text-white">
                        <div className="flex justify-between items-end">
                             ...
                             <div className={`px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1 ${isFree ? 'bg-emerald-500 text-white' : 'bg-white text-slate-900'}`}>
                ...
                <div className="p-4 flex flex-col flex-grow">
                    <h3 className="font-bold text-slate-900 mb-1 line-clamp-2 leading-snug group-hover:text-blue-600 transition-colors">{item.title}</h3>
    ...
    // RECRUIT STYLE
    return (
        <div onClick={onClick} className="group bg-white rounded-2xl border border-slate-100 overflow-hidden hover:shadow-xl transition-all cursor-pointer relative hover:-translate-y-1">
            <div className="relative h-56">
                <img src={item.img} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent"></div>
                <LikeButton />
                
                {/* Live Badge for Open Crew */}
                {!isEnded && (
            ...
            <div className="p-4 flex items-center justify-between bg-white">
                <div className="flex items-center gap-3">
                    <img src={crewItem.leaderProfile || "https://via.placeholder.com/40"} className="w-10 h-10 rounded-full border border-slate-200 object-cover" />
                    <div>
                        <p className="text-[10px] font-bold text-emerald-600">LEADER</p>
                        <p className="text-sm font-bold text-slate-800">{crewItem.leader}</p>
                    </div>
                </div>
  ...
  // --- 3. Lecture Card Style (YouTube Vibe) ---
    ...
    return (
        <div onClick={onClick} className="group cursor-pointer">
            ...
            <div className="flex gap-3">
                <img src={lectureItem.teacherProfile || "https://via.placeholder.com/40"} className="w-10 h-10 rounded-full object-cover mt-1 bg-slate-100" />
                <div>
                    <h3 className="font-bold text-slate-900 leading-snug mb-1 group-hover:text-blue-600 transition-colors line-clamp-2">{item.title}</h3>
                    <p className="text-sm text-slate-500 mb-1 line-clamp-1">{item.desc}</p>
  ...
  return (
    <div 
      onClick={onClick}
      className={`group bg-white rounded-xl border border-slate-200 overflow-hidden hover:shadow-lg transition-all duration-300 cursor-pointer flex flex-col h-full relative ${isEnded ? 'opacity-90' : 'hover:-translate-y-1'}`}
    >
      <div className="relative h-48 overflow-hidden">
        <img 
      ...
      <div className="p-5 flex flex-col flex-grow">
        <h3 className="font-bold text-lg text-slate-800 mb-2 line-clamp-2 leading-snug group-hover:text-amber-600 transition-colors">
          {item.title}
        </h3>
        ...
        <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-100">
            <span className={`font-bold ${item.price ? 'text-slate-900' : 'text-emerald-600'}`}>
            {item.price || '무료'}
            </span>
